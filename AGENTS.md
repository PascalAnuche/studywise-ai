...
- **Resources and Notes filter nothing** — the search fields, subject selects, sort selects and the Resources filter menu all hold their own state but do not filter, because there is no backend to filter. The exception is the Favorites tab on Notes, which uses the one flag the fixtures carry. Wire these up with the queries, not before.
- **Achievements runs on real rows now.** `lib/db/activity.ts` derives the milestones, the streak figures, the twelve-week heatmap and the weekly totals from `plan_sessions`, `explanations`, `quizzes` and `progress`. The invented fixtures that used to back them are deleted. Badges and Points Earned are the only mock figures left on that screen, because neither has a table; replace them there when one exists.
- **Profile and Settings were merged.** One sidebar entry, carrying Settings' label and icon, pointing at `/profile`; the separate `/settings` route is gone. Its Preferences and Privacy panels moved onto the tabs there rather than being deleted with it — the privacy disclosure is required by prompt section 12, not decoration. The page heading reads Settings, because a nav item and its destination disagreeing is a defect.
- **Flashcards was removed from the product.** The route, its fixtures, the sidebar entry and its icon are gone. `MOCK_STREAK_DAYS` moved to the Achievements fixtures, which is the only thing that still used it, and the figures that referenced decks on Profile and Achievements now name work the app actually does.
