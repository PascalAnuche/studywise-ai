...
- **Resources and Notes filter nothing** — the search fields, subject selects and sort selects on both hold their own state but do not filter, because there is no backend to filter. The exception is the Favorites tab on Notes, which uses the one flag the fixtures carry. Wire these up with the queries, not before.
